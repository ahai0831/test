...
>nul 2>&1 "%tools_7z_dir%7z.exe" --help &&(
...
if \".tar.gz\"==\"%zip_input_path:~-7%\" (
    7z.exe x "%zip_input_path%" -so | 7z.exe x -si -ttar -%overwrite_option% -o"%zip_output_path%" >nul
    goto final
)
@REM 处理*.tar.xz
...
@REM 处理其他一般格式的压缩包
7z.exe x "%zip_input_path%" -%overwrite_option% -o"%zip_output_path%" >nul
:final
exit /b
