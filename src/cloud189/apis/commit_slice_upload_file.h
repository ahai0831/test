...
// sliceMD5, [string],
// [表明所有分片的md5大写末尾加上\n拼接组成md5list，且最后片也加\n，并用Signature=hmac_sha1(MD5List,
// SessionSecret) 签名，再计算Signature的md5结果作为sliceMD5]
...
//
// 请求方式：POST
// header规定：Content—Type:
// application/x-www-form-urlencoded(安卓端为application/xml; charset=utf-8)
//
// 需要放到url中的参数：
//
// clientType, [string],
...
// sliceMD5, [string],
// [从json字符串中作为string解析，参数放到body中，表明所有分片的md5大写末尾加上\n拼接组成md5list，且最后片也加\n，
// 并用Signature=hmac_sha1(MD5List,SessionSecret) 签名，再计算Signature的md5结果作为sliceMD5]
//
// albumId,[int64_t],
// [表明上传图片至云相册，目标相册id,
