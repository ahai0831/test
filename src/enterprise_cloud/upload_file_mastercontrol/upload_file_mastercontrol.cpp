...
UploadFileMasterControl::UploadFileMasterControl(
    const std::string& file_path, int64_t corp_id, int64_t parent_id,
    const std::string& md5, int32_t file_source, const std::string coshare_id,
    int32_t is_log, const int32_t oper_type, const std::string& upload_file_id)
    : md5_finish_size(0),
      upload_bytes_(0),
      file_size_(0),
      file_ptr_(nullptr),
  ...
  assistant_ptr_ = std::make_unique<assistant::Assistant_v3>();
}
...
void UploadFileMasterControl::CRUFCallback(
  ...
  do {
    ...
    file_upload_id_ = atoll(json_value["uploadFileId"].asString().c_str());

    if (1 == json_value["fileDataExists"].asInt()) {
      // 跳第四步
      if (!COUFRequest()) {
        // TODO(sun): error
  ...
  do {
    ...
    auto json_params =
        EnterpriseCloud::Apis::GetUploadFileStatus::JsonStringHelper(
            atoll(upload_file_id_.c_str()), corp_id_, is_log_);
    if (json_params.empty()) {
      break;
    }
...
bool UploadFileMasterControl::UFDRequest() {
  ...
  do {
    ...
    auto json_params = EnterpriseCloud::Apis::UploadFileData::JsonStringHelper(
        file_upload_url_, file_path_, file_upload_id_, corp_id_, file_source_,
  ...
  do {
    ...
    auto json_params =
        EnterpriseCloud::Apis::ComfirmUploadFileComplete::JsonStringHelper(
            file_commit_url_, file_upload_id_, corp_id_, file_source_,
