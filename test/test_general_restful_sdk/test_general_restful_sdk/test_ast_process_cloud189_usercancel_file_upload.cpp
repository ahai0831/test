...
void test_ast_process_cloud189_usercancel_file_upload() {
  /// 初始化promise和future
  do {
  ...
  test_info_json["local_path"] =
      "/Volumes/File/天翼云盘/cloud189_corp_alpha_1.0.0_setup.dmg";
  ...
  test_usercancel_json["operation"] = "UserCancelUpload";
  ...
  AstProcess(test_usercancel_json_info.c_str(),
             [](const char *start_data) {
               ...
               printf("OnStart: %s%s\n",
                      0 != start_result ? "UserCancelUpload failed: " : "",
                      start_data);
             },
             nullptr);
