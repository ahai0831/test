...
struct ReadByMmap : public assistant::HttpResponse::CallbackBase {
public:
	bool count_size;
	/// x64下DWORD对应的类型是UL，应拆出两个DWORD，此处需要兼容
	const uint64_t filesize;
...
protected:
public:
	ReadByMmap(const char *filepath, int64_t size, int64_t begin, int64_t len)
		: count_size(false),
		filesize(size),
		begin_offset(begin),
		length(len),
		...
		do {
			...
			/// 指定为顺序访问而优化磁盘缓存
			file = CreateFileW(w_filepath.c_str(), GENERIC_READ,
			...
			/// 初始化文件映射内核对象句柄
			filemapping = CreateFileMappingW(
				file, NULL, PAGE_READONLY, static_cast<DWORD>(filesize >> 32),
				static_cast<DWORD>(filesize & 0xFFFFFFFF), NULL);
			if (NULL == filemapping) {
				break;
			}
	...
	static size_t Callback(void *ptr, size_t size, size_t nmemb, void *data) {
		auto &response = *static_cast<assistant::HttpResponse *>(data);
		auto &cb = static_cast<ReadByMmap &>(*response.transfer_callback);
		auto total_size = size * nmemb;
		/// 极限情况下，needed_length即write_size，理论上需兼容为ULL
		/// 但x86下，ULL对此值无意义，不会溢出，直接截断无影响
		size_t write_size = 0;
	...
	virtual void AccessDelegate(const uint64_t need_data_length, /*void(*callback)(void*, uint64_t)*/std::function<void(void*, uint64_t)> callback) final{
		...
		if (this->done + total_size <= this->length) {
			...
			/// 这种情况下将发生截断；
			needed_length = static_cast<decltype(needed_length)>(this->length - this->done);
		}
...
private:
	ReadByMmap() = delete;
	ReadByMmap(ReadByMmap const &) = delete;
	ReadByMmap &operator=(ReadByMmap const &) = delete;
	ReadByMmap(ReadByMmap &&) = delete;
};
