...
void MD5::update(uint1 *input, uint32_t input_length) {
...
std::string MD5::raw_string() {
  std::string raw_md5;
  if (0 != finalized) {
    raw_md5.resize(MD5_RES_LEN);
    memcpy((uint1 *)(raw_md5.data()), digest, MD5_RES_LEN);
  }
  return raw_md5;
}
char *MD5::hex_digest() {
...
std::string MD5::hex_string() {
  std::string hex_md5;
  if (0 != finalized) {
    hex_md5.reserve(MD5_RES_LEN * 2);
    memcpy((uint1 *)(hex_md5.data()), digest, MD5_RES_LEN);
    char buf[3] = {'\0'};
