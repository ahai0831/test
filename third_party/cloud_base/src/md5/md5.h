...
class MD5 {
  //摘要的结果的字节长度。
  const int32_t MD5_RES_LEN = 16;
  //摘要的结果转成ansi字符串后的字节长度：其结果是16个字节，每个字节用两个字符来表示16进制值。
  const int32_t MD5_RES_STR_LEN = 32;

 public:
  // methods for controlled operation:
  MD5();  // simple initializer
  void update(unsigned char *input, uint32_t input_length);
  ...
  /// Returned memory must be deleted!
