...
const TImageInfo* CGIFRender::LoadGIF(const LPBYTE pData, DWORD dwSize, DWORD mask/* = 0*/)
{
		...
		&frame.m_nDisposal))
	{
		...
		if (nullptr == pImage)
		{
			continue;  /// 由于部分GIF资源存在问题，特定帧的数据为空完全读不到，暂时先跳出这一帧的处理直接拿下一帧 2019.5.12
		}
		BITMAPINFO bmi;
...
const TImageInfo* CGIFRender::GetImage(int nIndex)   //获取指定桢的图像
{
	if (!m_pvFrames)
	{
		return NULL;
...
void CAnimationGIF::SetElapseTime(UINT uElapse /*= 50*/)  //设置刷新时间
{
	m_nElapse = uElapse;
}

void CAnimationGIF::SetCurAlpah(int nAlpha/* = 255*/)     // 设置当前显示的alpha值
{
	m_nAlpha = nAlpha;
}
