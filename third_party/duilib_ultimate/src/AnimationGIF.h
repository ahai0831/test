#ifndef _EUI_ANIMATION_GIF__H__
#define	_EUI_ANIMATION_GIF__H__
//管理GIF播放的类，用定时器实现
#include <vector>
using namespace std;
...
public:
	...
	const TImageInfo* GetImage(int nIndex);   //获取指定桢的图像
...
class UILIB_API CAnimationGIF :public CUIAnimation
{
public:
	CAnimationGIF(CControlUI* pOwner);
	~CAnimationGIF(void);
	...
	*/
	bool DrawAniImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CStdString& sImageName, \
	...
	void EmptyGIFHash();      //情况Gif管理类
	void InitCurFrame();   //初始化CurFram

	void SetElapseTime(UINT uElapse = 50);  // 设置刷新时间
	void SetCurAlpah(int nAlpha = 255);     // 设置当前显示的alpha值

	void SetLoopValue(bool bLoop = true);
	void SetLastFrame(bool bLastFrame = false);//需要与SetLoopValue配合使用	
protected:
	...
	UINT m_nAlpha;    //控制图片绘制的Alpha值

protected:
	CStdStringPtrMap    m_mGIFHash;  //管理GIF的类
	UINT     m_nElapse;      //刷新时间，以毫秒为单位
	bool m_bLoop;			//是否循环播放
};
}
#endif //_EUI_ANIMATION_GIF__H__