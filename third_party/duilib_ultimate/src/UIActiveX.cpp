...
void CActiveXUI::SetPos(RECT rc)
{
    ...
    if( !m_bCreated ) DoCreateControl(m_rcItem);
...
void CActiveXUI::SetDelayCreate(bool bDelayCreate)
{
    ...
    if( bDelayCreate == false ) {
        if( m_bCreated == false && m_clsid != IID_NULL ) DoCreateControl(m_rcItem);
    }
...
bool CActiveXUI::CreateControl(const CLSID clsid)
{
    ...
    m_bCreated = false;
    m_clsid = clsid;
    if( !m_bDelayCreate ) DoCreateControl(m_rcItem);
    return true;
}
...
bool CActiveXUI::DoCreateControl(RECT rc)
{
    ReleaseControl();
    ...
    if( (dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) == 0 ) {
        Hr = m_pUnk->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, pOleClientSite, 0, m_pManager->GetPaintWindow(), &rc);
