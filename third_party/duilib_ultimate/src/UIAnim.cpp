...
#define PI 3.1415926535897932384626433832795029L
	...
	bool CAnimationSpooler::RenderJob_Flat(const CAnimJobUI* pJob, LPDIRECT3DSURFACE9 /*pSurface*/, DWORD dwTick)
	{
		...
		HRESULT Hr = 0;
		for( int iBuffer = pJob->iBufferStart; (iBuffer >= 0 && iBuffer < pJob->iBufferEnd && iBuffer < MAX_BUFFERS); iBuffer++ ) {
			// Lock the vertex buffer and apply transformation
#if _DEBUG
			if(iBuffer > MAX_BUFFERS)
			{
				CStdString str;
				str.Format(_T("m_nBuffers = %d iBuffer = %d,m_aJobs size = %d"),m_nBuffers,iBuffer,m_aJobs.GetSize());
				MessageBox(NULL,str.GetData(),NULL,MB_OK);
			}
#endif
