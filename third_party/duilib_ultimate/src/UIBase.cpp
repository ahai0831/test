...
// 去掉左边的空格
void CStdString::TrimLeft()
{
	LPCTSTR pstrTemp = m_pstr;
...
//     ASSERT(m_hWnd==NULL);
    m_OldWndProc = SubclassWindow(hWnd, CtrlWndProc);
...
void CWindowWnd::ResizeWindow(bool bShow)   // 根据浏览器对比位置，重置在当前浏览器中的位置
{
	if (bShow)    // 根据窗口的位置，重新计算所在显示器的位置
	{
		HWND hParent = GetParent(m_hWnd);
		if ((hParent == NULL) || !IsWindow(hParent))
		{
			HMONITOR hWndMonitor = MonitorFromWindow(m_hWnd, MONITOR_DEFAULTTONEAREST);
			HMONITOR hMainMonitor = CPaintManagerUI::GetMainMonitor();
			if (hMainMonitor != NULL)
			{
				if (hMainMonitor != hWndMonitor)    //非同一个浏览器下
				{
					MONITORINFO miMain;
					miMain.cbSize = sizeof(miMain);
					GetMonitorInfo(hMainMonitor, &miMain);
					RECT rcArea = miMain.rcMonitor;

					MONITORINFO miWnd;
					miWnd.cbSize = sizeof(miWnd);
					GetMonitorInfo(hWndMonitor, &miWnd);
					RECT rcArea1 = miWnd.rcMonitor;

					RECT rcWnd = { 0 };
					::GetWindowRect(m_hWnd, &rcWnd);

					int DlgWidth = rcWnd.right - rcWnd.left;
					int DlgHeight = rcWnd.bottom - rcWnd.top;
					// Find dialog's upper left based on rcCenter
					int xLeft = (rcWnd.left - rcArea1.left) * ((rcArea.right - rcArea.left) / (rcArea1.right - rcArea1.left)) + rcArea.left;
					int yTop = (rcWnd.top - rcArea1.top) * ((rcArea1.bottom - rcArea1.top) / (rcArea.bottom - rcArea.top)) + rcArea.top;
...
void CWindowWnd::CenterWindow()
{
	...
	if (hWndCenter == NULL || (hWndCenter && IsIconic(hWndCenter)))
	{
		HMONITOR hMonitor = CPaintManagerUI::GetMainMonitor();
		if ((hMonitor == NULL) || (hMonitor == INVALID_HANDLE_VALUE))
		{
			// 为空时，取主要的显示器大小区域
			::SystemParametersInfo(SPI_GETWORKAREA, NULL, &rcArea, NULL);
		}
		else if (hMonitor != INVALID_HANDLE_VALUE)
		{
			MONITORINFO mi;
			mi.cbSize = sizeof(mi);
...
// 	CStdString str2;
// 	str2.Format(_T(" rcArea.left=%d, rcArea.top=%d, rcArea.right=%d, rcArea.bottom=%d"), rcArea.left, rcArea.top, rcArea.right, rcArea.bottom);
// 	::MessageBox(NULL, str2.GetData(), _T(""), MB_OK);
// 	CStdString str;
// 	str.Format(_T("Ower:%d, rect rcCenter.left=%d, rcCenter.top=%d, rcCenter.right=%d, rcCenter.bottom=%d"), &hWndCenter, rcCenter.left, rcCenter.top, rcCenter.right, rcCenter.bottom);
// 	::MessageBox(NULL, str.GetData(), _T(""), MB_OK);
...
// 	CStdString str1;
// 	str1.Format(_T("show rect x=%d, y=%d, DlgWidth=%d, DlgHeight=%d"), xLeft, yTop, DlgWidth, DlgHeight);
// 	::MessageBox(NULL, str1.GetData(), _T(""), MB_OK);

    ::SetWindowPos(m_hWnd, NULL, xLeft, yTop, -1, -1, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
}

CPoint CWindowWnd::CenterWindowPoint(int nWidth, int nHeight)
{
	RECT rcArea = { 0 };
	RECT rcCenter = { 0 };
	...
	rcCenter = rcArea;
	// Find dialog's upper left based on rcCenter
	int xLeft = (rcCenter.left + rcCenter.right) / 2 - nWidth / 2;
	int yTop = (rcCenter.top + rcCenter.bottom) / 2 - nHeight / 2;

	// The dialog is outside the screen, move it inside
	if (xLeft < rcArea.left) xLeft = rcArea.left;
	else if (xLeft + nWidth > rcArea.right) xLeft = rcArea.right - nWidth;
	if (yTop < rcArea.top) yTop = rcArea.top;
	else if (yTop + nHeight > rcArea.bottom) yTop = rcArea.bottom - nHeight;
	return CPoint(xLeft, yTop);
}
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_NCCREATE ) {
        ...
        if( (uMsg == WM_NCDESTROY) && (pThis != NULL) && ::IsWindow(pThis->m_hWnd)) {
...
LRESULT CALLBACK CWindowWnd::CtrlWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	CWindowWnd* pThis = NULL;
	if( uMsg == WM_NCCREATE ) {
		LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);
...
void CWindowWnd::RightBottomWindow()
{
	...
	// Find dialog's upper left based on rcCenter
	int xLeft = rcArea.right - DlgWidth;
	int yTop = rcArea.bottom - DlgHeight;
...
void CWindowWnd::ShellTraySideWindow(bool bTopMost/* = false*/)    // 在托盘区域附近显示窗口
{
	...
	HWND hWndTrayNotifyWnd = FindWindowEx(hWndTray, 0, _T("TrayNotifyWnd"), NULL);
	if (hWndTrayNotifyWnd == NULL)
	{
		return;
	}

	HWND hWndOverflow = FindWindow(_T("NotifyIconOverflowWindow"), NULL);  // 隐藏图标窗口
	RECT rcOverflow;     
	bool bOverFlowShow = false;   // 隐藏图标窗口是否显示
	if (hWndOverflow != NULL && IsWindowVisible(hWndOverflow))
	{
		GetWindowRect(hWndOverflow, &rcOverflow);
		bOverFlowShow = true;
	}
	RECT rcShellTray;
	::GetWindowRect(hWndTray, &rcShellTray);
	RECT rcTrayNotify;
	::GetWindowRect(hWndTrayNotifyWnd, &rcTrayNotify);
	...
	int xLeft = 0;
	int yTop = 0;
	if ((rcShellTray.top > 0) && (rcShellTray.left == 0))  // 任务栏在底部
	{
		if (bOverFlowShow)    // 隐藏图标窗口显示，向心显示
		{
			xLeft = rcOverflow.left - DlgWidth;
		}
		else
			xLeft = rcArea.right - DlgWidth;
		yTop = rcArea.bottom - DlgHeight;
	...
	else if ((rcTrayNotify.right <= rcArea.left) && (rcShellTray.top == 0) && (rcShellTray.left == 0))  // 任务栏在左侧
	{
		xLeft = rcArea.left;
		if (bOverFlowShow)  // 隐藏图标窗口显示，向心显示
		{
			yTop = rcOverflow.top - DlgHeight;
		}
		else
			yTop = rcArea.bottom - DlgHeight;
	}
	else if ((rcTrayNotify.bottom <= rcArea.top) && (rcShellTray.top == 0) && (rcShellTray.left == 0))   // 任务栏在顶部
	{
		if (bOverFlowShow)  // 隐藏图标窗口显示，向心显示
		{
			xLeft = rcOverflow.left - DlgWidth;
	...
	else if ((rcShellTray.top == 0) && (rcShellTray.left > 0))   // 任务栏在右侧
	{
		xLeft = rcArea.right - DlgWidth;
		if (bOverFlowShow)  // 隐藏图标窗口显示，向心显示
		{
	...
	if (bTopMost)
	{
		::SetWindowPos(m_hWnd, HWND_TOPMOST, xLeft, yTop, -1, -1, SWP_NOSIZE | SWP_NOACTIVATE);
	}
	else
		::SetWindowPos(m_hWnd, NULL, xLeft, yTop, -1, -1, SWP_NOSIZE | SWP_NOACTIVATE);
