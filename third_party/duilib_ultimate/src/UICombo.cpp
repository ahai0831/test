...
void CComboWnd::Init(CComboUI* pOwner)
{
	...
	RECT rcDropInset = m_pOwner->GetDropWndInset();
	cyFixed += rcDropInset.top + rcDropInset.bottom;
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        m_pLayout->SetInset(m_pOwner->GetDropWndInset());
...
CComboUI::CComboUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0)
{
	...
	m_rcDropWndInset = CRect(2, 2, 2, 2);
...
bool CComboUI::SelectItem(int iIndex, bool bTakeFocus /* = false */)
{
...
bool CComboUI::GetDropTogether() const
{
	return m_bDropTogether;
}
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("droptogether")) == 0)
	{
		if (_tcsstr(pstrValue, _T("true")) != NULL) {
			m_bDropTogether = true;
		}
	}
	...
	else if (_tcscmp(pstrName, _T("dropinset")) == 0) {
		RECT rcInset = { 0 };
		LPTSTR pstr = NULL;
...
RECT CComboUI::GetDropWndInset() const
{
	return m_rcDropWndInset;
}
