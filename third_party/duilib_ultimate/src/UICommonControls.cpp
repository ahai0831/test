...
CLabelUI::CLabelUI() : m_uTextStyle(DT_VCENTER), m_dwTextColor(1), 
    m_dwDisabledTextColor(1), m_iFont(-1), m_bShowHtml(false), m_bLinkStype(false)
{
...
void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("font")) == 0 )
	{
		...
		m_iNormalFont = _ttoi(pstrValue);
		m_iHoverFont = m_iNormalFont;
	}
	else if ( _tcscmp(pstrName, _T("hoverfont")) == 0)
	{
		m_iHoverFont = _ttoi(pstrValue);
	}
	...
	else if (_tcscmp(pstrName, _T("linkstyle")) == 0)
	{
		if (_tcscmp(pstrValue, _T("true")) == 0)
		{
			m_bLinkStype = true;
	...
	else if (_tcscmp(pstrName, _T("linespace")) == 0)
	{
		m_iLineSpace = _ttoi(pstrValue);
	}
...
void CLabelUI::PaintText(HDC hDC)
{
	...
	{
		CRenderEngine::DrawMultLineText(hDC, m_pManager, rc, m_iLineSpace, m_sText, m_dwTextColor, m_iFont, m_uTextStyle);
	}

}

void CLabelUI::Setlinkstyle( bool blinkstyle )
{
	m_bLinkStype = blinkstyle;
}
...
COptionUI::COptionUI() : m_bSelected(false), m_dwSelectedTextColor(0), m_bShowForeImage(false), m_bAutocalcwidth(false)
{
}
...
void COptionUI::Selected(bool bSelected, bool bSendmessage/* = true*/)
{
    ...
    if( m_pManager != NULL ) {
        if( !m_sGroupName.IsEmpty() ) {
			...
			else
			{
				if (bSendmessage)
				{
					m_pManager->SendNotify(this, _T("unselectchanged"), DUILIB_OP_SELECTCHANGED, m_bSelected);
				}
			}
        }
        else {
...
void COptionUI::SetShowForeImage(bool bShow /* = false */)
{
	if (m_bShowForeImage != bShow)
	{
...
bool COptionUI::GetShowForeImage() const
{
	return m_bShowForeImage;
}
...
void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("needforeimage")) == 0)
	{
		if(_tcscmp(pstrValue, _T("true")) == 0)
		{
			SetShowForeImage(true);
		}
		else
		{
			SetShowForeImage(false);
		}
	}
...
Label_ForeImage:
	if(m_bShowForeImage && !m_sForeImage.IsEmpty() ) {
...
void COptionUI::SetAutoCalcWidth(bool bAuto /*= false*/)
{
	m_bAutocalcwidth = bAuto;
}

bool COptionUI::GetAutoCalcWidth() const
{
...
void CTextUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONUP && IsEnabled() ) {
        for( int i = 0; i < m_nLinks; i++ ) {
            if( ::PtInRect(&m_rcLinks[i], event.ptMouse) ) {
                m_pManager->SendNotify(this, _T("link"), DUILIB_TEXT_LINK, i);
                return;
            }
        }
    }
...
void CSliderUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONUP )
    {
        ...
        m_pManager->SendNotify(this, _T("valuechanged"), DUILIB_SLIDER_VALUECHANGED);
        Invalidate();
        return;
    }
    if( event.Type == UIEVENT_CONTEXTMENU )
...
RECT CEditWnd::CalPos()   //计算位置
{
    CRect rcPos = m_pOwner->GetPos();
//	::MapWindowRect(m_pOwner->GetManager()->GetPaintWindow(), HWND_DESKTOP, &rcPos);
...
m_bPasswordMode(false),m_bNumberOnly(false), m_cPasswordChar(_T('*')), m_uButtonState(0), m_dwEditbkColor(0xFFFFFFFF)
{
    SetTextPadding(CRect(4, 3, 4, 3));
    SetBkColor(0xFFFFFFFF);
	m_dwDefaultTextColor = 0xFF393939;
}
...
void CEditUI::SetSel()
{
	if (m_pWindow)
	{
		Edit_SetSel(m_pWindow->GetHWND(), 0 , -1);
	}
}
...
void CEditUI::SetDefaultText(LPCTSTR pStrDefaultText)
{
	m_strDefaultText = pStrDefaultText;
}

LPCTSTR CEditUI::GetDefaultText()
{
...
void CEditUI::PaintText(HDC hDC)
{
    ...
    if( m_sText.IsEmpty() )
	{
		...
		{
			CRenderEngine::DrawText(hDC, m_pManager, rc, m_strDefaultText, m_dwDefaultTextColor, \
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
    {
        ...
        if( m_pManager != NULL && m_pOwner == NULL ) m_pManager->SendNotify(this, _T("scroll"), DUILIB_SCROLLBAR_SCROLL);
        return;
    }
    if( event.Type == UIEVENT_BUTTONUP )
    {
    ...
    if( (event.Type == UIEVENT_MOUSELEAVE) && ((m_uThumbState & UISTATE_CAPTURED) == 0) )
    {
        if( IsEnabled() ) {
            m_uButton1State &= ~UISTATE_HOT;
            m_uButton2State &= ~UISTATE_HOT;
...
void CScrollBarUI::PaintBk(HDC hDC)
{
	...
	if (m_bHorizontal)
	{
		m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), m_rcButton1.right - m_rcItem.left, \
			m_rcThumb.top - m_rcItem.top, m_rcButton2.left - m_rcItem.left, m_rcThumb.bottom - m_rcItem.top);
	}
	else
		m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), m_rcThumb.left - m_rcItem.left, \
		m_rcButton1.bottom - m_rcItem.top, m_rcThumb.right - m_rcItem.left, m_rcButton2.top - m_rcItem.top);

    if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
        if( !m_sBkDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sBkDisabledImage, (LPCTSTR)m_sImageModify))
        ...
        if( !m_sBkPushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sBkPushedImage, (LPCTSTR)m_sImageModify)) 
        ...
        if( !m_sBkHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sBkHotImage, (LPCTSTR)m_sImageModify))
				m_sBkHotImage.Empty();
            else return;
        }
    }
    ...
    if( !m_sBkNormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sBkNormalImage, (LPCTSTR)m_sImageModify))
