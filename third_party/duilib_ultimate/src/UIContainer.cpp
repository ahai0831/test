...
CControlUI* CContainerUI::FindControl(FINDCONTROLPROC Proc, LPVOID pData, UINT uFlags)
{
    ...
    if( (uFlags & UIFIND_TOP_FIRST) != 0 ) {
        for( int it = m_items.GetSize() - 1; it >= 0; it-- ) {
			CControlUI *pControlParent = static_cast<CControlUI*>(m_items[it]);
			CControlUI* pControl = NULL;
			if (pControlParent)
			{
				pControl = pControlParent->FindControl(Proc, pData, uFlags);
			}
            if( pControl != NULL ) {
...
void CContainerUI::SetAllNeedHeight(int nHeight)
{ 
	if (nHeight < (m_rcItem.bottom - m_rcItem.top))
	{
		m_nAllNeedHeight = 0;
		return;
	}
	m_nAllNeedHeight = nHeight;
}

int CContainerUI::GetAllNeedHeight()
{
	return m_nAllNeedHeight;
}
...
void CContainerUI::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired)
{
	...
	//add by ligthen
	if ( (m_nAllNeedHeight != 0) && (cyRequired < m_nAllNeedHeight))   //如果用户设置数据且大于实际需要数据
	{
		cyRequired = m_nAllNeedHeight;
	}
	else if (m_nAllNeedHeight != 0)   //用户设置的才修改数据
	{
		m_nAllNeedHeight = cyRequired;
	}
...
void CVerticalLayoutUI::DoPostPaint(HDC hDC, const RECT& rcPaint)
{
    if( (m_uButtonState & UISTATE_CAPTURED) != 0 && !m_bImmMode ) {
		...
		CRenderEngine::DrawColor(hDC, m_pManager, rcSeparator, 0xAA000000);
    }
}
...
bool CTabLayoutUI::RemoveAt(int iIndex)
{
	...
	CControlUI* pControl = GetItemAt(iIndex);
	if (pControl != NULL) {
		ret =  CContainerUI::Remove(pControl);
	}
	if (ret)
	{
		if (m_iCurSel == iIndex)
...
//         m_pManager->SendNotify(this, _T("tabselect"), DUILIB_TAB_SELECT, m_iCurSel, iOldSel);
//     }
//     return true;
// }

bool CTabLayoutUI::SelectItem(int iIndex, bool bFocus /* = true */)
{
	if( iIndex < 0 || iIndex >= m_items.GetSize() ) return false;
	if( iIndex == m_iCurSel ) return true;
