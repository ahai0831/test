...
{
    return CRenderEngine::DrawImageString(hDC, m_pManager, m_bitampParam, m_rcItem, m_rcPaint, pStrImage, pStrModify);
}

bool CControlUI::DrawImage(HDC hDC, LPCTSTR pStrImage, const RECT &rcpos, LPCTSTR pStrModify /* = NULL */)
{
	RECT rcTemp;
	if (::IntersectRect(&rcTemp, &rcpos, &m_rcItem))
	{
		return CRenderEngine::DrawImageString(hDC, m_pManager, m_bitampParam, rcTemp, m_rcPaint, pStrImage, pStrModify);
	}
	return false;
}
...
UINT CControlUI::GetToolTipMaxWidth() const
{
	return m_nToolTipMaxWidth;
}

//设置tooltip窗口最大长度，控制多行显示
void CControlUI::SetToolTipMaxWidth(UINT nMaxWidth)
{
	m_nToolTipMaxWidth = nMaxWidth;
}
...
// 位图绘制参数
void CControlUI::SetBitmapRotation(double rotation)       // 设置图像绘制角度，用于旋转
{
	if (rotation < 0)
	{
		rotation = 0;
	...
	m_bitampParam.nRotation = rotation;
}

double CControlUI::GetBitmapRotation() const
{
	return m_bitampParam.nRotation;
}

void CControlUI::SetBitmapZoom(bool bZoom)
{
	m_bitampParam.bZoom = bZoom;
}

bool CControlUI::GetBitmapZoom()const
{
	return m_bitampParam.bZoom;
}
...
void CControlUI::DoEvent(TEventUI& event)
{
    ...
    {
        m_pManager->SendNotify(this, _T("timer"), DUILIB_TIMER, event.wParam, event.lParam);
        return;
    }
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("zoomout")) == 0) SetBitmapZoom(_tcscmp(pstrValue, _T("true")) == 0);
	...
	else if (_tcscmp(pstrName, _T("airlayout")) == 0) setAirLayout(_tcscmp(pstrValue, _T("true")) == 0);
}
...
void CControlUI::PaintBkColor(HDC hDC)
{
    if( m_dwBackColor != 0 ) {
		...
		else if (m_dwBackColor >= 0xFF000000) CRenderEngine::DrawColor(hDC, m_pManager, m_rcPaint, GetAdjustColor(m_dwBackColor));
		else CRenderEngine::DrawColor(hDC, m_pManager, m_rcItem, GetAdjustColor(m_dwBackColor));
    }
}
...
//add by lighten
bool CControlUI::SetMCaptured(CControlUI *pControl)
{
	bool bOldStatus = m_bMCaptured;
	m_pManager->SetMCaptured(pControl);
	m_bMCaptured = true;
	return bOldStatus;
}

bool CControlUI::ReleaseMCaptured()
{
	bool bOldStatus = m_bMCaptured;
	m_bMCaptured = false;
	m_pManager->ReleaseMCapture();
	return bOldStatus;
}

bool CControlUI::IsMCaptured() const
{
	return m_bMCaptured;
}
	...
	// Returns:   UiLib::CStdString
	// Qualifier: const
	// Note:	  
	//************************************
	CStdString CControlUI::GetEffectStyle() const
	{
		try
		{
		...
		try
		{
			try
			{
				...
				if(GetManager() && m_bEnabledEffect && pcTEffect->m_bEnableEffect)
					GetManager()->AddAnimJob(CAnimJobUI(UIANIMTYPE_FLAT,0,pcTEffect->m_iNeedTimer,pcTEffect->m_dFillingBK,pcTEffect->m_dFillingBK,GetPos(),pcTEffect->m_iOffectX,pcTEffect->m_iOffectY,pcTEffect->m_iZoom,pcTEffect->m_iAlpha,(float)pcTEffect->m_fRotation));
			}
			catch (...)
			{
	...
	bool CControlUI::IsAirLayout()
	{
		return m_bAirLayout;
	}

	void CControlUI::setAirLayout(bool bAirLayout /*= false*/)
	{
		m_bAirLayout = bAirLayout;
	}
