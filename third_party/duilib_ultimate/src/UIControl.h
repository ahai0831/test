...
public:
	...
	//在指定位置绘制图像 add by lighten
	bool DrawImage(HDC hDC, LPCTSTR pStrImage, const RECT &rcpos, LPCTSTR pStrModify = NULL);   
	...
	virtual UINT GetToolTipMaxWidth() const;
	virtual void SetToolTipMaxWidth(UINT nMaxWidth);   //设置tooltip窗口最大长度，控制多行显示
	...
	virtual bool IsAirLayout();
	virtual void setAirLayout(bool bAirLayout = false);

	// 位图绘制参数
	virtual void SetBitmapRotation(double rotation);       // 设置图像绘制角度，用于旋转
	virtual double GetBitmapRotation() const;
	virtual void SetBitmapZoom(bool bZoom);					//是否需要把图片缩小
	virtual bool GetBitmapZoom()const;
	...
	//add by lighten
	virtual bool SetMCaptured(CControlUI *pControl);
	virtual bool ReleaseMCaptured();
	virtual bool IsMCaptured() const;

	virtual CStdString GetEffectStyle() const;
	...
	bool AddAnimationJob( const CAnimJobUI & job );
private:
	bool		m_bEnabledEffect;
	CStdString	m_strEffectStyle;
...
protected:
	...
	bool m_bMCaptured;   //是否鼠标要截取

	BitmapParam m_bitampParam;   // 图像绘制参数
	bool m_bAirLayout;
};
