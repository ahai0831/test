...
typedef struct BitmapParam    // 图像绘制参数
{
	BitmapParam()
	{
		nRotation = 0;
...
typedef struct tagTImageInfo
{
    ...
    CStdString sResType;
    DWORD dwMask;
	HBITMAP hTmpBitmap;   // 存储临时图像处理后的位图
	BitmapParam bitmapParam;   // 图像处理参数
} TImageInfo;
...
// 自定义消息，绘制主界面，提供消息，供用户自定义绘制界面背景
// wParam : HDC

#define  WM_PAINT_BACKGROUND   WM_USER + 20000
...
public:
	...
	void SetTransparentAndColor(int nOpacity, DWORD dwColor = 0);   //add by lighten 2013.03.18

	//add by lighten
// 	typedef BOOL (WINAPI *MYFUNC)(HWND,HDC,POINT*,SIZE*,HDC,POINT*,COLORREF,BLENDFUNCTION*,DWORD);   
	void SetUpdateTransparent(bool bUpdateLayered = true, int nAlpha = 255);   //设置透明
	bool m_bUpdateLayered;
	bool m_bTranslate;
	...
	static HMONITOR GetMainMonitor();
	...
	static void SetMainMonitor(HMONITOR  hMonitor);
//     static void SetResourceZip(LPCTSTR pStrZip);
	...
	void  AddRotationImage(LPCTSTR bitmap, HBITMAP hBitmap, double nRotation);   //加入旋转的临时数据
	...
	CStdString GetEffectsStyle(LPCTSTR pStrStyleName);
	...
	//add by lighten
	void SetMCaptured(CControlUI *pControl);
	CControlUI *GetMCapture() const;
	void ReleaseMCapture();

	bool IsSetUpdatelayered() const;
// 	void ADDToPostPaint(CControlUI *pControl);
// 	void RemoveAtFromPostPaint(CControlUI *pControl);


    bool AddNotifier(INotifyUI* pControl);
    bool RemoveNotifier(INotifyUI* pControl);   
    void SendNotify(TNotifyUI& Msg, bool bAsync = false);
    void SendNotify(CControlUI* pControl, LPCTSTR pstrMessage, DWORD dwMessageID = 0, WPARAM wParam = 0, LPARAM lParam = 0, bool bAsync = false);
	...
	static void DlgMessageLoop(volatile BOOL & bEndLoop);
...
private:
	...
	CControlUI* m_pLastHover;   //tooltip重绘控制
    CControlUI* m_pEventClick;
    CControlUI* m_pEventKey;

	//add by lighten
	CControlUI* m_pCapturedEvent;
