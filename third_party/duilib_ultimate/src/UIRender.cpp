...
//    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	...
	ASSERT(dwDCType == OBJ_DC || dwDCType == OBJ_MEMDC);
...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( type == NULL ) {
        ...
        if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
			//add by lighten 区分决定路径
			CStdString strFilePath = bitmap.m_lpstr;
			if (strFilePath.Find(_T(":")) >= 0)  //绝对路径
			{
				sFile = strFilePath;
			}
			else    //相对路径
	...
	LPCTSTR   lpExt=(LPCTSTR)_tcsrchr(bitmap.m_lpstr, _T('.'));
	if(!lpExt)
		return NULL; 
	lpExt++; 
	...
	case   _T( 'p '): 
		{
			lpExt++;
			if((*lpExt) == _T('n') || (*lpExt) == _T('N'))
			{
				imageType = CXIMAGE_FORMAT_PNG; 
			}
			else
			{
				imageType = CXIMAGE_FORMAT_PSD; 
			}
		}	
		break; 
	case   _T( 'B '): 
	...
	CxImage Img;
	bool bRet = Img.Decode((BYTE*)pData,dwSize,imageType);
	if(!bRet)
	{
	...
	y = Img.GetHeight();
	TImageInfo* data = new TImageInfo;
	HBITMAP hBitmap = Img.MakeBitmap_();
	data->hBitmap = hBitmap;
	data->nX = x;
	data->nY = y;
	data->alphaChannel = Img.AlphaIsValid();
...
bool DrawImage(HDC hDC, CPaintManagerUI* pManager, BitmapParam & bitmapParam, const RECT& rc, const RECT& rcPaint, const CStdString& sImageName, \
		...
		bool bHole, bool bTiledX, bool bTiledY)
{
	...
	HBITMAP hBitmap = data->hBitmap;
	bool zoom = false;
	if(bitmapParam.bZoom)
	{
		//目前用于预览图片，把超过绘画区域大小的图片，按比率压缩成合适的大小，确保不失真。
		int nPaintWidth = rcItem.right - rcItem.left;
		int nPaintHeight = rcItem.bottom - rcItem.top;
		if(nPaintWidth > nPaintHeight )
		{
			if(nPaintWidth < data->nX)
			{
				int ndestWidth = nPaintWidth;
				int ndestHeight = (int)(((double)ndestWidth/(double)data->nX)* (double)data->nY);
				CxImage Img;
				//根据位图信息加载图片有可能会失败
				if (Img.CreateFromHBITMAP(hBitmap, 0, data->alphaChannel))
				{
					Img.Resample(ndestWidth, ndestHeight, 3);
					hBitmap = Img.MakeBitmap_();
					zoom = true;
					if (rcBmpPart.right > ndestWidth) rcBmpPart.right = ndestWidth;
					if (rcBmpPart.bottom > ndestHeight) rcBmpPart.bottom = ndestHeight;
				}
			}
		}
		else
		{
			if(nPaintHeight < data->nY)
			{

				int ndestHeight = nPaintHeight;
				int ndestWidth = (int)(((double)ndestHeight/(double)data->nY) * (double)data->nX);
				CxImage Img;
				//根据位图信息加载图片有可能会失败
				if (Img.CreateFromHBITMAP(hBitmap, 0, data->alphaChannel))
				{
	...
	if (bitmapParam.nRotation != 0 )
	{
		// fixed： 旋转图像目前不支持透明度绘制
		if (data->bitmapParam.nRotation != bitmapParam.nRotation)
		{
	       HBITMAP hTmpBitmap = aarot::rotate(hBitmap, bitmapParam.nRotation, NULL, false);
		   pManager->AddRotationImage((LPCTSTR)sImageName, hTmpBitmap, bitmapParam.nRotation);
		}
		BITMAP bitmapinfo;
		memset(&bitmapinfo, 0x00, sizeof(BITMAP));
		::GetObject(data->hTmpBitmap, sizeof(BITMAP), &bitmapinfo);
		
		rcBmpPart.left = rcBmpPart.top = 0;
		rcBmpPart.right = bitmapinfo.bmWidth;
		rcBmpPart.bottom = bitmapinfo.bmHeight;
		if (rcBmpPart.right > bitmapinfo.bmWidth) rcBmpPart.right = bitmapinfo.bmWidth;
		if (rcBmpPart.bottom > bitmapinfo.bmHeight) rcBmpPart.bottom = bitmapinfo.bmHeight;

		CRenderEngine::DrawImage(hDC, data->hTmpBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, false, bFade, bHole, bTiledX, bTiledY);
	}
	else
	{
		//pManager->AddRotationImage((LPCTSTR)sImageName, NULL, 0);
		CRenderEngine::DrawImage(hDC, hBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, data->alphaChannel, bFade, bHole, bTiledX, bTiledY);
	}
	//绘画完后释放压缩后位图句柄
	if (zoom)
	{
...
bool CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, BitmapParam & bitmapParam, const RECT& rc, const RECT& rcPaint, 
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						DuiLib::DrawImage(hDC, pManager, bitmapParam, rc, rcPaint, sImageName, sImageResType,
...
void CRenderEngine::DrawColor(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, DWORD color)
{
	...
	if (!pManager->IsSetUpdatelayered())
	{
		Graphics graphics(hDC);
		// Create a Rect object.
		Rect fillRect(rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top);
		if (color >= 0xFF000000)
		{
			// Create a SolidBrush object.
			SolidBrush blackBrush(Color(255, GetBValue(color), GetGValue(color), GetRValue(color)));
			// Fill the rectangle.
		...
		{
			// Create a SolidBrush object.
			SolidBrush blackBrush(Color(color >> 24, GetBValue(color), GetGValue(color), GetRValue(color)));
			// Fill the rectangle.
			graphics.FillRectangle(&blackBrush, fillRect);
		}
	}
	else
	{
		...
		{
			// Create a SolidBrush object.
			SolidBrush blackBrush(Color(254, GetBValue(color), GetGValue(color), GetRValue(color)));
			// Fill the rectangle.
			graphics.FillRectangle(&blackBrush, fillRect);
		}
		else
...
//    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	...
	else
	{
		...
		if (hFont != NULL)
		{
			...
			SolidBrush blackBrush(Color(254, GetBValue(dwTextColor), GetGValue(dwTextColor), GetRValue(dwTextColor)));
			StringFormat format;
			if (uStyle & DT_LEFT)
			{
			...
			// Draw string.
			graph.DrawString(pstrText, -1, &myFont,	layoutRect,	&format, &blackBrush);	
		}

	}
}

void CRenderEngine::DrawMultLineText(HDC hDC, CPaintManagerUI* pManager, RECT& rect, int nLineSpace, CStdString pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
	if (pstrText.GetLength() <= 0)
		return;

	WCHAR* pText = (WCHAR*)pstrText.GetData();
	int nTextLength = pstrText.GetLength();
	RECT rtChar;
	::SetBkMode(hDC, TRANSPARENT);
	::SetTextColor(hDC, RGB(GetBValue(dwTextColor), GetGValue(dwTextColor), GetRValue(dwTextColor)));
	HFONT hOldFont = (HFONT)::SelectObject(hDC, pManager->GetFont(iFont));
	...
	int nCount = 0;
	int nLineCount = 0;
	int nLeaveLen = nTextLength;
	int nCharWidth = 0;
	int nLineCharArray[10] = { 0 };
	// 统计函数
	while (nCount < nTextLength)
	{
		int nLineChar = 0;   // 行显示的字符数
		SIZE size;
		long lRet = ::GetTextExtentExPoint(hDC, pText + nCount, nLeaveLen, nWidth, &nLineChar, NULL, &size);
		if (lRet)
		{
			CStdString strLine = pstrText.Mid(nCount, nLineChar);
			nCount += nLineChar;
			nLeaveLen -= nLineChar;
			if (uStyle & DT_BOTTOM)
			{
				nCharWidth = size.cy;
				nBottom = nBottom - nLineCount * nLineSpace - nCharWidth;
				if (nBottom < (nTop - nCharWidth))  // 允许最后一行超出范围的显示
				{
					break;
				}
				nLineCount++;
				nLineCharArray[nLineCount] = nLineChar;
			}
			else
			{
				rcText.top = nTop + nLineCount * nLineSpace;
				rcText.bottom = rcText.top + size.cy;
				nTop = rcText.bottom;
				if (nTop > nBottom + nCharWidth)  // 允许最后一行超出范围的显示
				{
					break;
				}
				nLineCount++;
				::DrawText(hDC, strLine, -1, &rcText, DT_SINGLELINE | uStyle | DT_NOPREFIX);
			}
		}
		else   // 出错，直接返回
		{
	...
	if (uStyle & DT_BOTTOM)
	{
		...
		{
			rcText.bottom = nBottom - nLine * nLineSpace;
			rcText.top = rcText.bottom - nCharWidth;
			nBottom = rcText.top;
			if (nBottom < (nTop - nCharWidth))  // 允许最后一行超出范围的显示
			{
				break;
			...
			nLine++;
			int nchar = nLineCharArray[i];
			CStdString strLine;
			if (nCount == 0)
			{
				strLine = pstrText.Mid(nchar, nTextLength - nchar);
			}
			else
			{
				strLine = pstrText.Mid(nchar, nCount);
			}
			nCount = nchar;
...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CStdString* sLinks, int& nLinkRects, UINT uStyle)
{
	...
	//	 NoUnderlineInLink:<v x>text</v>		same as link but without underline;2018.11.30 wtf;
    ...
    while( *pstrText != _T('\0') ) {
        if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
            ...
            case _T('u'):  // Underline text
                ...
                break;
			case  _T('v'):		//link without underline;
			{
				pstrText++;
				while (*pstrText > _T('\0') && *pstrText <= _T(' ')) pstrText = ::CharNext(pstrText);
				if (iLinkIndex < nLinkRects && !bLineDraw) {
