...
HRESULT InitDefaultCharFormat(CRichEditUI* re, CHARFORMAT2W* pcf, HFONT hfont) 
{
	...
	if (re && re->GetManager())
	{
		yPixPerInch = ::GetDeviceCaps(re->GetManager()->GetPaintDC(), LOGPIXELSY);
	}
//     LONG yPixPerInch = GetDeviceCaps(re->GetManager()->GetPaintDC(), LOGPIXELSY);
	if (yPixPerInch == 0)
	{
		pcf->yHeight = -lf.lfHeight * LY_PER_INCH / 900;
	}
	else 
...
HRESULT CTxtWinHost::CreateRichEdit(ITextHost *pTextHost, IUnknown **ppUnk)
{
   HRESULT hr = E_FAIL;
...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
    ...
    if(FAILED(CreateRichEdit(this, &pUnk)))
        goto err;
...
void CTxtWinHost::SetFont(HFONT hFont) 
{
	...
	{
		yPixPerInch = ::GetDeviceCaps(m_re->GetManager()->GetPaintDC(), LOGPIXELSY);
	}
	if (yPixPerInch == 0)
	{
		cf.yHeight = -lf.lfHeight * LY_PER_INCH / 900;
	}
	else
...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
    ...
    m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_nLineSpacing(0), m_bLinkStype(false), m_iNormalFont(-1), m_iHoverFont(-1)
{
}
    ...
    if( m_pTwh ) {
        if( msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN ) {
            if( !m_bWantReturn || (::GetKeyState(VK_CONTROL) < 0 && !m_bWantCtrlReturn) ) {
                if( m_pManager != NULL ) m_pManager->SendNotify((CControlUI*)this, _T("return"), DUILIB_EDIT_VK_RETURN);
                return S_OK;
            }
        }
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    {
		if (::PtInRect(&m_rcItem, event.ptMouse) && m_pManager && m_bLinkStype)
		{
...
//		pf2.dwMask |= PFM_LINESPACING| PFM_OFFSET|PFM_STARTINDENT|PFM_RIGHTINDENT;
...
void CRichEditUI::Setlinkstyle( bool blinkstyle )
{
