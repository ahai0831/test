...
bool DuiLib::CWebBrowserUI::DoCreateControl(RECT rc)
{
	if (!CActiveXUI::DoCreateControl(rc))
		return false;
...
// 		break;
	case DISPID_PROGRESSCHANGE:
		{
			ProgressChange(pDispParams->rgvarg[1].lVal,
...
void DuiLib::CWebBrowserUI::Navigate2( LPCTSTR lpszUrl )
{
	...
	{
		m_pWebBrowserEventHandler->Navigate2(lpszUrl);
	}
//	OutputDebugStr(_T("CWebBrowserUI::Navigate2 \n"));
}
...
//	OutputDebugStr(_T("CWebBrowserUI::BeforeNavigate2 \n"));
...
//	OutputDebugStr(_T("CWebBrowserUI::NavigateComplete2 \n"));
}
...
void DuiLib::CWebBrowserUI::ProgressChange( LONG nProgress, LONG nProgressMax )
{
	...
	CStdString str;
	str.Format(_T("CWebBrowserUI::ProgressChange  now=%d, max =%d\n"), nProgress, nProgressMax);
//	OutputDebugStr(str);
}
...
void DuiLib::CWebBrowserUI::DocumentCompleted()
{
	if (m_pWebBrowserEventHandler)
	{
		m_pWebBrowserEventHandler->DocumentCompleted();
	}
//	OutputDebugStr(_T("CWebBrowserUI::DocumentCompleted \n"));
}
...
HRESULT DuiLib::CWebBrowserUI::RunScript( LPCTSTR lpszScript, VARIANT* pvarRet )
{
	HRESULT hr = E_FAIL;
	...
	if (m_pWebBrowser2)
	{
		...
		m_pWebBrowser2->get_Document((IDispatch **)&pDoc2);
		if (pDoc2)
		{
			CComPtr<IHTMLWindow2> pWindow2;
			pDoc2->get_parentWindow(&pWindow2);

			if (pWindow2)
			{
				BSTR bstrHTML = SysAllocString(lpszScript);
				BSTR bstrLng = SysAllocString(_T("jscript"));
				hr = pWindow2->execScript(bstrHTML, bstrLng, pvarRet);
				SysFreeString(bstrHTML);
				SysFreeString(bstrLng);
			}
		}
	}
